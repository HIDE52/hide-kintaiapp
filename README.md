...
⓷「Docker Desktop 」の確認を行い、「hide-kintaiapp」コンテナが作成されているか確認を行います。
...
※アカウント共通情報
一般ユーザー・管理者ともに、ログインパスワードはすべて<br/> `password` で統一されています。
...
・アカウント権限： 管理者
・検証できる内容（特徴）：
  管理者専用の勤怠一覧画面にアクセスできます。
  全従業員の毎日の打刻実績、休憩時間、実働時間の集計結果をまとめて閲覧できるほか、一般ユーザーから提出された「修正申請」の承認・却下フローの検証が可能です。
