...
・アカウント権限： 管理者
・検証できる内容（特徴）：
  管理者専用の勤怠一覧画面（ダッシュボード）にアクセスできます。全従業員の毎日の打刻実績、休憩時間、実働時間の集計結果をまとめて閲覧できるほか、一般ユーザーから提出された「修正申請」の承認・却下フローの検証が可能です。
...
* **検証できる内容（特徴）：**
  **【打刻忘れ・未退勤状態の検証】**
  昨日分の勤怠データに「20:00に出勤打刻したが、退勤打刻をしないまま日付を跨いだ（`punch_out` が `null`）」という状態が再現されています。画面上での「本日分の出勤ボタン」の制御や、未退勤データの表示崩れ、警告表示の確認に最適です。
...
* **検証できる内容（特徴）：**
  **【修正申請・承認待ち状態の検証】**
  昨日分のデータに対して、すでに管理者へ「修正申請（ステータス：承認待ち）」を提出済みの状態が再現されています。一般ユーザー画面での「申請中」のステータス表示や、二重申請のブロック制御、管理者側への通知確認の検証に最適です。
