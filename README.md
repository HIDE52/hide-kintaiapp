...
### 👥 テスト用アカウント・動作確認用データ一覧

本アプリケーションでは、様々な勤怠パターンの集計や管理者による修正申請の承認フローをスムーズに検証できるよう、以下の特徴を持ったテストユーザーが自動生成されます。

各画面での動作確認の際、以下のメールアドレスと共通パスワードを用いてログインしてください。

| 氏名 | メールアドレス | 共通パスワード | アカウント権限 | 検証・動作確認できるエッジケース（特徴） |
| :--- | :--- | :--- | :--- | :--- |
| 管理者（総務部） | `admin@example.com` | `password` | **管理者** | 管理者専用ダッシュボードの閲覧、全従業員の勤怠一覧確認、一般ユーザーからの修正申請の承認・却下フローの検証 |
| 佐藤 肇 | `userA@example.com` | `password` | 一般ユーザー | **【複数休憩の集計検証】**<br>当月15日のデータに、1日に2回休憩を取得した（12:00〜12:45、15:00〜15:15）実績データが挿入されます。休憩時間の合計や実働時間が正しく集計されているかの確認に最適です。 |
| 鈴木 涼子 | `userB@example.com` | `password` | 一般ユーザー | **【打刻忘れ・24時超え勤務の検証】**<br>昨日分のデータに「20:00に出勤打刻したが、退勤打刻がない（`punch_out` が `null`）」という状態が挿入されます。画面上での「退勤ボタン」の制御や、未退勤データの表示崩れがないかの確認に最適です。 |
| 高橋 健太 | `userC@example.com` | `password` | 一般ユーザー | **【修正申請（承認待ち）の検証】**<br>昨日分のデータに対して、すでに管理者へ「修正申請（ステータス：承認待ち）」を送っている状態が挿入されます。一般画面での申請中ステータス表示や、管理者画面への申請通知の検証に最適です。 |
