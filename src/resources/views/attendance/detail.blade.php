            ...
            <form action="/attendance/detail/{{ $attendance->id }}" method="POST" novalidate>
                ...
                <tbody>
                    ...
                    <tr class="attendance-detail-table__tr">
                        ...
                        <td class="attendance-detail-table__td attendance-detail-table__content" colspan="2">
                    ...
                    <tr class="attendance-detail-table__tr">
                        ...
                        <td class="attendance-detail-table__td attendance-detail-table__content" colspan="2">
                            <div class="attendance-detail__date-group">
                                <span class="attendance-detail__text-year">{{ $carbonDate->format('Y年') }}</span>
                                <span class="attendance-detail__text-month-day">{{ $carbonDate->format('m月d日') }}</span>
                            </div>
                        </td>
                    </tr>
                    ...
                    <tr class="attendance-detail-table__tr">
                        ...
                        <td class="attendance-detail-table__td attendance-detail-table__content">
                            <div class="attendance-detail__time-group">
                                <input type="text" name="punch_in" class="attendance-detail__input-time" value="{{ old('punch_in', \Carbon\Carbon::parse($attendance->punch_in)->format('H:i')) }}" {{ $isWaiting ? 'readonly' : '' }}>
                                <span class="attendance-detail__separator">〜</span>
                                <input type="text" name="punch_out" class="attendance-detail__input-time" value="{{ old('punch_out', $attendance->punch_out ? \Carbon\Carbon::parse($attendance->punch_out)->format('H:i') : '') }}" {{ $isWaiting ? 'readonly' : '' }}>
                            </div>
                            @error('punch_in')
                    ...
                    <tr class="attendance-detail-table__tr">
                        ...
                        <td class="attendance-detail-table__td attendance-detail-table__content">
                            <div class="attendance-detail__time-group">
                                <input type="text" name="rest_id[{{ $index }}][break_in]" class="attendance-detail__input-time" value="{{ old('rest_id.' . $index . '.break_in', \Carbon\Carbon::parse($rest->break_in)->format('H:i')) }}" {{ $isWaiting ? 'readonly' : '' }}>
                                <span class="attendance-detail__separator">〜</span>
                                <input type="text" name="rest_id[{{ $index }}][break_out]" class="attendance-detail__input-time" value="{{ old('rest_id.' . $index . '.break_out', $rest->break_out ? \Carbon\Carbon::parse($rest->break_out)->format('H:i') : '') }}" {{ $isWaiting ? 'readonly' : '' }}>
                            </div>
                    ...
                    <tr class="attendance-detail-table__tr">
                        ...
                        <td class="attendance-detail-table__td attendance-detail-table__content" colspan="2">
                            <textarea name="remark" class="attendance-detail__textarea" {{ $isWaiting ? 'readonly' : '' }}>{{ old('remark', $attendance->note) }}</textarea>
                            @error('remark')
                                <div class="attendance-detail__error-message">{{ $message }}</div>
                            @enderror
                        </td>
